...
def approx(dimtype, d, onepluseps):
    """
    Compute the nearest neighbours for each of the query tweets
    using various speedups tricks.
    ...
    onepluseps : float
        0.5 or 0.2 (max. tolerance of the distance)
    Returns
    -------
    ...
    for j, q_tweet in enumerate(query_tweets):
        ...
        for cands, count in reversed(zip(counts_to_candidates, range(1, len(counts_to_candidates) + 1))):
            # distance bound:
            distance_bound = comps.get_lower_bound(
                q_tweet_size, count) * onepluseps
            if distance_bound >= cur_min_angle:
                break
            for cand in cands:
                db_tweet = set(database_tweets[cand].split())
                # assert len(db_tweet.intersection(q_tweet)) == count
...
if __name__ == '__main__':
    ...
    onepluseps = settings.eps_list[int(sys.argv[3])]

    print dimtype, d, onepluseps

    tweet_indices, angles, t = approx(dimtype, d, onepluseps)
    # transformations due to pypy stuff:
    out_dict = {'time': t,
                'tweet_indices': [int(ind) for ind in tweet_indices],
                'angles': [float(ang) for ang in angles]}

    out_fname = settings.approx_fnames[int(sys.argv[3])][dimtype][d]
